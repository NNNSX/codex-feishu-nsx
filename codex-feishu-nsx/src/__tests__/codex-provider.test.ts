...
async function captureThreadOptions(permissionMode?: string): Promise<Record<string, unknown>> {
  ...
  await collectStream(provider.streamChat({
    prompt: 'sandbox test',
    sessionId: `sandbox-${permissionMode || 'default'}-${Date.now()}`,
    permissionMode,
    workingDirectory: 'D:\\codex',
  }));
  assert.ok(captured, 'startThread options should be captured');
  return captured;
}
...
describe('CodexProvider', () => {
  ...
  it('maps image_generation_call item to an attachment SSE event', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const events = parseSSEChunks(chunks);
    assert.equal(events.length, 1);
    assert.equal(events[0].type, 'attachment');
    const attachment = JSON.parse(events[0].data);
    assert.equal(attachment.id, 'ig-test');
    assert.equal(attachment.name, 'ig-test.png');
    assert.equal(attachment.type, 'image/png');
    assert.equal(attachment.data, pngBase64);
  });

  it('emits a visible attachment error for malformed generated-image data', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    const chunks: string[] = [];
    ...
    const events = parseSSEChunks(chunks);
    assert.equal(events.length, 1);
    assert.equal(events[0].type, 'attachment_error');
    assert.match(events[0].data, /malformed/i);
  });

  it('recovers image_generation_call from the current Codex rollout', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    const pngBase64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==';
    const threadId = '019f92a4-1fdb-7493-b691-e9a20005fb65';
    const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cti-codex-home-'));
    const sessionsDir = path.join(codexHome, 'sessions', '2026', '07', '24');
    fs.mkdirSync(sessionsDir, { recursive: true });
    const rolloutPath = path.join(sessionsDir, `rollout-test-${threadId}.jsonl`);
    ...
    try {
      const chunks = await collectStream(provider.streamChat({ prompt: 'draw', sessionId: 'rollout-image-session' }));
      const events = parseSSEChunks(chunks);
      const attachmentEvent = events.find(e => e.type === 'attachment');
      assert.ok(attachmentEvent, 'Should emit an attachment recovered from the rollout');
      const attachment = JSON.parse(attachmentEvent.data);
      assert.equal(attachment.id, 'ig-rollout');
      assert.equal(attachment.data, pngBase64);
    } finally {
  ...
  it('does not pass model by default and still attempts resume for persisted thread ids', async () => {
    ...
    const stream = provider.streamChat({
      ...
      sdkSessionId: 'stored-session-id',
      model: 'stored-model',
    });
    ...
    assert.equal(resumedThreadId, 'stored-session-id');
  ...
  it('reuses the in-memory Codex thread when a stored model exists', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
  ...
  it('passes model only when CFN_CODEX_PASS_MODEL=true', async () => {
    const old = process.env.CFN_CODEX_PASS_MODEL;
    process.env.CFN_CODEX_PASS_MODEL = 'true';
    try {
      const { CodexProvider } = await import('../codex-provider.js');
      const { PendingPermissions } = await import('../permission-gateway.js');
      const provider = new CodexProvider(new PendingPermissions());
      ...
      if (old === undefined) {
        delete process.env.CFN_CODEX_PASS_MODEL;
      } else {
        process.env.CFN_CODEX_PASS_MODEL = old;
      }
    }
  });

  it('passes skipGitRepoCheck only when CFN_CODEX_SKIP_GIT_REPO_CHECK=true', async () => {
    const old = process.env.CFN_CODEX_SKIP_GIT_REPO_CHECK;
    process.env.CFN_CODEX_SKIP_GIT_REPO_CHECK = 'true';
    try {
      const { CodexProvider } = await import('../codex-provider.js');
      const { PendingPermissions } = await import('../permission-gateway.js');
      const provider = new CodexProvider(new PendingPermissions());
      ...
      if (old === undefined) {
        delete process.env.CFN_CODEX_SKIP_GIT_REPO_CHECK;
      } else {
        process.env.CFN_CODEX_SKIP_GIT_REPO_CHECK = old;
      }
    }
  });

  it('uses workspace-write only for code mode by default', async () => {
    const old = process.env.CFN_CODEX_SANDBOX_MODE;
    delete process.env.CFN_CODEX_SANDBOX_MODE;
    try {
      const codeOptions = await captureThreadOptions('acceptEdits');
      const planOptions = await captureThreadOptions('plan');
      const askOptions = await captureThreadOptions('default');

      assert.equal(codeOptions.sandboxMode, 'workspace-write');
      assert.equal(planOptions.sandboxMode, 'read-only');
      assert.equal(askOptions.sandboxMode, 'read-only');
    } finally {
      if (old === undefined) delete process.env.CFN_CODEX_SANDBOX_MODE;
      else process.env.CFN_CODEX_SANDBOX_MODE = old;
    }
  });

  it('allows an explicit safe Codex sandbox override', async () => {
    const old = process.env.CFN_CODEX_SANDBOX_MODE;
    process.env.CFN_CODEX_SANDBOX_MODE = 'read-only';
    try {
      const options = await captureThreadOptions('acceptEdits');
