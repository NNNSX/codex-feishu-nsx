...
import { initBridgeContext } from '../core/context';
import type { BridgeJobRecord, BridgeStore, LifecycleHooks } from '../core/host';
import type { BaseChannelAdapter } from '../core/channel-adapter';
...
describe('bridge-manager lifecycle', () => {
  ...
  it('getStatus returns not running when bridge has not started', async () => {
    ...
    const { getStatus } = await import('../core/bridge-manager');
  ...
  it('maps adapter transport health into bridge status', async () => {
    const manager = await import('../core/bridge-manager');
...
describe('bridge-manager durable recovery', () => {
  ...
  it('keeps failed delivery pending and completes it after the channel recovers', async () => {
    ...
    const { _testOnly } = await import('../core/bridge-manager');
