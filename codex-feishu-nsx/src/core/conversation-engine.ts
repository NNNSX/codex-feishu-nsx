...
function cleanupExpiredUploads(uploadDirectory: string): void {
...
function buildProviderPrompt(
  ...
  if (persistedFiles.length > 0) {
    const fileList = persistedFiles.map((file) =>
      `- ${JSON.stringify(file.name)} (${file.type}, ${file.size} bytes): ${file.filePath}`
    );
    sections.push([
...
export function collectOutboxAttachments(
  responseText: string,
  workingDirectory: string,
): { text: string; attachments: FileAttachment[]; warnings: string[] } {
...
export async function processMessage(
  ...
  try {
    ...
    if (files && files.length > 0) {
      if (workingDirectory) {
        try {
          ...
          const fileMeta: FileAttachment[] = files.map((f) => {
    ...
    if (workingDirectory) {
      try { fs.mkdirSync(path.join(workingDirectory, OUTBOX_DIRECTORY), { recursive: true }); } catch { /* provider can still reply with text */ }
    }
...
async function consumeStream(
  ...
  let currentText = '';
  const responseAttachments: FileAttachment[] = [];
  ...
  try {
    while (true) {
      ...
      for (const line of lines) {
        ...
        switch (event.type) {
          ...
          case 'attachment': {
            try {
              const attachment = JSON.parse(event.data) as FileAttachment;
              if (
                attachment.id &&
                attachment.name &&
          ...
          case 'attachment_error':
            if (event.data) attachmentWarnings.push(event.data);
            break;
    ...
    responseAttachments.push(...outboxResult.attachments.filter(
      (candidate) => !responseAttachments.some((existing) => existing.id === candidate.id),
    ));
    const responseText = [
      outboxResult.text,
