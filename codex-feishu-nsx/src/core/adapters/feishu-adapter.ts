...
function feishuFileKind(attachment: FileAttachment): { fileType: FeishuFileType; messageType: 'file' | 'audio' | 'media' } {
  const ext = fileExtension(attachment.name);
  if (attachment.type === 'audio/opus' || ext === 'opus') return { fileType: 'opus', messageType: 'audio' };
  if (attachment.type === 'video/mp4' || ext === 'mp4') return { fileType: 'mp4', messageType: 'media' };
  if (attachment.type === 'application/pdf' || ext === 'pdf') return { fileType: 'pdf', messageType: 'file' };
  if (['doc', 'docx'].includes(ext)) return { fileType: 'doc', messageType: 'file' };
  if (['xls', 'xlsx'].includes(ext)) return { fileType: 'xls', messageType: 'file' };
  if (['ppt', 'pptx'].includes(ext)) return { fileType: 'ppt', messageType: 'file' };
  return { fileType: 'stream', messageType: 'file' };
}

function mimeFromFileName(name: string, fallback: string): string {
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  private connectionState: 'idle' | 'connecting' | 'connected' | 'reconnecting' | 'failed' = 'idle';
  private reconnectAttempts = 0;
  private lastConnectionChangeAt: string | null = null;
   ...
   */
  onMessageStart(chatId: string, taskId?: string): void {
    const messageId = this.lastIncomingMessageId.get(chatId);
    if (taskId) this.currentTaskIds.set(chatId, taskId);
  ...
  private async handleCardAction(data: unknown): Promise<unknown> {
    ...
    try {
      ...
      if (accepted) this.enqueue(callbackMsg);
   ...
   */
  private createStreamingCard(chatId: string, replyToMessageId?: string, taskId?: string): Promise<boolean> {
  ...
  private async _doCreateStreamingCard(chatId: string, replyToMessageId?: string, taskId?: string): Promise<boolean> {
  ...
  private flushCardUpdate(chatId: string): void {
    ...
    if (state.taskId) getBridgeContext().store.updateBridgeJob?.(state.taskId, { streamingSequence: seq });
  ...
  private async finalizeCard(
    ...
    try {
      ...
      state.sequence++;
      if (state.taskId) getBridgeContext().store.updateBridgeJob?.(state.taskId, { streamingSequence: state.sequence });
  ...
  async finalizeRecoveredTask(job: import('../host.js').BridgeJobRecord): Promise<void> {
    if (!this.restClient || !job.streamingCardId || job.streamingCardFinalized) return;
  ...
  async send(message: OutboundMessage): Promise<SendResult> {
    ...
    if (message.attachments && message.attachments.length > 0) {
      const attachmentResult = await this.sendAttachments(
        message.address.chatId,
        message.attachments,
        message.replyToMessageId,
        message.idempotencyKey,
      );
      if (!attachmentResult.ok || !message.text.trim()) return attachmentResult;
    }
  ...
  private async sendAttachments(
    ...
    for (const attachment of attachments) {
      try {
        ...
        if (!buffer) return { ok: false, error: `Invalid or empty attachment data: ${attachment.name}` };

        let messageType: 'image' | 'file' | 'audio' | 'media';
        let content: string;
        if (attachment.type.startsWith('image/')) {
          ...
          if (!upload?.image_key) return { ok: false, error: 'Feishu image upload returned no image_key' };
          messageType = 'image';
          ...
          if (!upload?.file_key) return { ok: false, error: 'Feishu file upload returned no file_key' };
   ...
   */
  private async sendAsCard(chatId: string, text: string, replyToMessageId?: string, idempotencyKey?: string): Promise<SendResult> {
   ...
   */
  private async sendAsPost(chatId: string, text: string, replyToMessageId?: string, idempotencyKey?: string): Promise<SendResult> {
  ...
  private async processIncomingEvent(data: FeishuMessageEventData): Promise<void> {
    ...
    if (trimmedText.startsWith('/perm ')) {
      ...
      if (permParts.length >= 3) {
        ...
        if (accepted) this.enqueue(inbound);
        return;
      }
    }
  ...
  private extractResourceMetadata(content: string): { fileKey: string | null; fileName?: string } {
    try {
      const parsed = JSON.parse(content);
      return {
