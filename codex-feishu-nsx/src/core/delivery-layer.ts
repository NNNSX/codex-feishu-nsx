...
function attachmentSummary(attachments?: FileAttachment[]): string {
  if (!attachments?.length) return '';
  return `[attachments] ${attachments.map((item) => item.name).join(', ')}`;
}

function attachmentDeliveryKey(
  channelType: string,
  chatId: string,
...
export async function deliverAttachments(
  adapter: BaseChannelAdapter,
  message: Omit<OutboundMessage, 'text' | 'attachments' | 'idempotencyKey'> & {
  ...
  for (const attachment of message.attachments) {
    const deliveryKey = attachmentDeliveryKey(
      adapter.channelType,
...
export async function deliverRendered(
  ...
  chunks: Array<{ html: string; text: string }>,
