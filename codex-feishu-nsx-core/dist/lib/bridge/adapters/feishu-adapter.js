 ...
 * Routes messages through an internal async queue.
 *
...
import { htmlToFeishuMarkdown, preprocessFeishuMarkdown, hasComplexMarkdown, buildCardContent, buildPostContent, buildStreamingContent, buildFinalCardJson, buildPermissionButtonCard, formatElapsed, } from '../markdown/feishu.js';
/** Max number of message_ids to keep for dedup. */
const DEDUP_MAX = 1000;
/** Max file download size (20 MB). */
const MAX_FILE_SIZE = 20 * 1024 * 1024;
/** Feishu's message-image upload limit. */
const MAX_OUTBOUND_IMAGE_SIZE = 10 * 1024 * 1024;
/** Feishu's message-file upload limit. */
...
function decodeBase64(data) {
    ...
    return canonical === normalized.replace(/=+$/, '') ? buffer : null;
}
...
function safeFileName(name) {
    ...
    return (sanitized || 'attachment.bin').slice(0, 180);
}
function fileExtension(name) {
    const dot = name.lastIndexOf('.');
    return dot >= 0 ? name.slice(dot + 1).toLowerCase() : '';
}
function feishuFileKind(attachment) {
    const ext = fileExtension(attachment.name);
    if (attachment.type === 'audio/opus' || ext === 'opus')
        return { fileType: 'opus', messageType: 'audio' };
    if (attachment.type === 'video/mp4' || ext === 'mp4')
        return { fileType: 'mp4', messageType: 'media' };
    if (attachment.type === 'application/pdf' || ext === 'pdf')
...
function mimeFromFileName(name, fallback) {
    ...
    return byExtension[fileExtension(name)] || fallback;
}
export class FeishuAdapter extends BaseChannelAdapter {
    ...
    async start() {
        ...
        this.running = true;
        // Restore locally persisted events before accepting new WebSocket events.
        for (const pending of getBridgeContext().store.listInboundMessages?.('feishu') || []) {
            this.addToDedup(pending.messageId);
        ...
        this.lastConnectionChangeAt = new Date().toISOString();
        ...
        this.wsClient = new lark.WSClient({
            ...
            onReady: () => {
                ...
                this.lastConnectionChangeAt = new Date().toISOString();
                this.lastConnectionError = null;
                console.log('[feishu-adapter] WebSocket connected');
            },
            onReconnecting: () => {
                this.connectionState = 'reconnecting';
                this.reconnectAttempts += 1;
                this.lastConnectionChangeAt = new Date().toISOString();
                console.warn('[feishu-adapter] WebSocket reconnecting (attempt', this.reconnectAttempts, ')');
            },
            onReconnected: () => {
                this.connectionState = 'connected';
                this.reconnectAttempts = 0;
                this.lastConnectionChangeAt = new Date().toISOString();
                this.lastConnectionError = null;
                console.log('[feishu-adapter] WebSocket reconnected');
            },
            onError: (error) => {
                this.connectionState = 'failed';
                this.lastConnectionChangeAt = new Date().toISOString();
                this.lastConnectionError = error.message;
                console.error('[feishu-adapter] WebSocket connection failed:', error.message);
            },
        ...
        if (typeof wsClientAny.handleEventData === 'function') {
            ...
            console.warn('[feishu-adapter] SDK card callback compatibility hook is unavailable; use /perm text commands as fallback');
        }
        try {
    ...
    getConnectionHealth() {
        ...
        return {
            state: status?.state || (this.running ? this.connectionState : 'idle'),
            reconnectAttempts: status?.reconnectAttempts ?? this.reconnectAttempts,
     ...
     */
    onMessageStart(chatId, taskId) {
        const messageId = this.lastIncomingMessageId.get(chatId);
        if (taskId)
            this.currentTaskIds.set(chatId, taskId);
        // Create streaming card (fire-and-forget — fallback to traditional if fails)
        if (messageId) {
            this.createStreamingCard(chatId, messageId, taskId).catch(() => { });
        }
    ...
    async handleCardAction(data) {
        const FALLBACK_TOAST = { toast: { type: 'info', content: '已收到' } };
        try {
            const event = data;
            ...
            const accepted = getBridgeContext().store.enqueueInboundMessage?.(callbackMsg) ?? true;
            if (accepted)
     ...
     */
    createStreamingCard(chatId, replyToMessageId, taskId) {
        if (!this.restClient || this.activeCards.has(chatId))
        ...
        if (existing)
            return existing;
        const promise = this._doCreateStreamingCard(chatId, replyToMessageId, taskId || this.currentTaskIds.get(chatId));
        this.cardCreatePromises.set(chatId, promise);
        promise.finally(() => this.cardCreatePromises.delete(chatId));
        return promise;
    }
    async _doCreateStreamingCard(chatId, replyToMessageId, taskId) {
        if (!this.restClient)
            return false;
        try {
            ...
            if (taskId) {
                getBridgeContext().store.updateBridgeJob?.(taskId, {
    ...
    flushCardUpdate(chatId) {
        ...
        if (state.taskId)
            getBridgeContext().store.updateBridgeJob?.(state.taskId, { streamingSequence: seq });
    ...
    async finalizeCard(chatId, status, responseText) {
        ...
        if (pending) {
            ...
            catch { /* creation failed — no card to finalize */ }
        }
        const state = this.activeCards.get(chatId);
        if (!state || !this.restClient)
            return false;
        ...
        try {
            ...
            state.sequence++;
            if (state.taskId)
                getBridgeContext().store.updateBridgeJob?.(state.taskId, { streamingSequence: state.sequence });
            await this.restClient.cardkit.v1.card.settings({
                path: { card_id: state.cardId },
                data: {
                    settings: JSON.stringify({ config: { streaming_mode: false } }),
                    sequence: state.sequence,
                    uuid: `s_${state.cardId}_${state.sequence}`,
                },
            });
            ...
            await this.restClient.cardkit.v1.card.update({
                ...
                data: {
                    ...
                    sequence: state.sequence,
                    uuid: `u_${state.cardId}_${state.sequence}`,
                },
            });
            console.log(`[feishu-adapter] Card finalized: cardId=${state.cardId}, status=${status}, elapsed=${formatElapsed(elapsedMs)}`);
            if (state.taskId) {
                // Commit the visible-card and text-delivered markers together so a
                // crash after the Feishu update cannot cause a duplicate text reply.
                getBridgeContext().store.updateBridgeJob?.(state.taskId, {
    ...
    onStreamText(chatId, fullText) {
        if (!this.activeCards.has(chatId)) {
            ...
            const messageId = this.lastIncomingMessageId.get(chatId);
            this.createStreamingCard(chatId, messageId, this.currentTaskIds.get(chatId)).then((ok) => {
                if (ok)
    ...
    async finalizeRecoveredTask(job) {
        if (!this.restClient || !job.streamingCardId || job.streamingCardFinalized)
            return;
        let sequence = (job.streamingSequence || 0) + 1;
        await this.restClient.cardkit.v1.card.settings({
            path: { card_id: job.streamingCardId },
            data: {
                settings: JSON.stringify({ config: { streaming_mode: false } }),
                sequence,
                uuid: `recover_s_${job.streamingCardId}_${sequence}`,
            },
        });
        sequence++;
        const startedAt = job.streamingStartedAt ? Date.parse(job.streamingStartedAt) : Date.now();
        const interrupted = job.state === 'interrupted';
        const failed = job.state === 'failed' || job.hasError;
        const finalCardJson = buildFinalCardJson(interrupted || job.state === 'failed' ? '' : job.responseText, [], {
            status: interrupted ? '⚠️ Interrupted' : failed ? '❌ Error' : '✅ Completed',
            elapsed: formatElapsed(Math.max(0, Date.now() - startedAt)),
        });
        await this.restClient.cardkit.v1.card.update({
            path: { card_id: job.streamingCardId },
            data: {
                card: { type: 'card_json', data: finalCardJson },
                sequence,
                uuid: `recover_u_${job.streamingCardId}_${sequence}`,
            },
        });
        getBridgeContext().store.updateBridgeJob?.(job.id, {
            streamingSequence: sequence,
            streamingCardFinalized: true,
            textDelivered: interrupted || job.state === 'failed' ? job.textDelivered : true,
        });
    }
    // ── Send ────────────────────────────────────────────────────
    async send(message) {
        ...
        if (message.attachments && message.attachments.length > 0) {
            const attachmentResult = await this.sendAttachments(message.address.chatId, message.attachments, message.replyToMessageId, message.idempotencyKey);
            if (!attachmentResult.ok || !message.text.trim())
        ...
        // Preprocess markdown for Codex responses
        ...
        if (message.inlineButtons && message.inlineButtons.length > 0) {
            return this.sendPermissionCard(message.address.chatId, text, message.inlineButtons, message.replyToMessageId, message.idempotencyKey);
        }
        // Rendering strategy (aligned with Openclaw):
        // - Code blocks / tables → interactive card (schema 2.0 markdown)
        // - Other text → post (md tag)
        if (hasComplexMarkdown(text)) {
            return this.sendAsCard(message.address.chatId, text, message.replyToMessageId, message.idempotencyKey);
        }
        return this.sendAsPost(message.address.chatId, text, message.replyToMessageId, message.idempotencyKey);
    }
    async sendAttachments(chatId, attachments, replyToMessageId, idempotencyKey) {
        let lastMessageId;
        for (const attachment of attachments) {
            try {
                const buffer = decodeBase64(attachment.data);
                if (!buffer)
                    return { ok: false, error: `Invalid or empty attachment data: ${attachment.name}` };
                let messageType;
                let content;
                if (attachment.type.startsWith('image/')) {
                    if (buffer.length > MAX_OUTBOUND_IMAGE_SIZE) {
                        return { ok: false, error: `Image exceeds Feishu's 10 MB limit: ${attachment.name}` };
                    }
                    if (!isSupportedImage(buffer)) {
                        return { ok: false, error: `Unsupported or mislabeled image data: ${attachment.name}` };
                    }
                    const upload = await this.restClient.im.image.create({
                        data: { image_type: 'message', image: buffer },
                    });
                    if (!upload?.image_key)
                        return { ok: false, error: 'Feishu image upload returned no image_key' };
                    messageType = 'image';
                    content = JSON.stringify({ image_key: upload.image_key });
                }
                else {
                    if (buffer.length > MAX_OUTBOUND_FILE_SIZE) {
                        return { ok: false, error: `File exceeds Feishu's 30 MB limit: ${attachment.name}` };
                    }
                    const kind = feishuFileKind(attachment);
                    const upload = await this.restClient.im.file.create({
                        data: {
                            file_type: kind.fileType,
                            file_name: safeFileName(attachment.name),
                            file: buffer,
                        },
                    });
                    if (!upload?.file_key)
                        return { ok: false, error: 'Feishu file upload returned no file_key' };
                ...
                const sent = replyToMessageId
                    ? await this.restClient.im.message.reply({
                        path: { message_id: replyToMessageId },
                        data: { msg_type: messageType, content, uuid: idempotencyKey },
                    })
                ...
                lastMessageId = sent?.data?.message_id;
                if (!lastMessageId) {
                    return { ok: false, error: 'Feishu attachment message returned no message_id' };
                }
                console.log(`[feishu-adapter] Attachment sent: name=${attachment.name}, type=${messageType}, bytes=${buffer.length}, msgId=${lastMessageId}`);
            }
            catch (err) {
                const error = err instanceof Error ? err.message : String(err);
                console.warn('[feishu-adapter] Attachment upload/send failed:', error);
     ...
     */
    async sendAsCard(chatId, text, replyToMessageId, idempotencyKey) {
        const cardContent = buildCardContent(text);
        try {
            const res = replyToMessageId
                ? await this.restClient.im.message.reply({
                    path: { message_id: replyToMessageId },
                    data: { msg_type: 'interactive', content: cardContent, uuid: idempotencyKey },
                })
                : await this.restClient.im.message.create({
                    params: { receive_id_type: 'chat_id' },
                    data: {
        ...
        // Fallback to post
        return this.sendAsPost(chatId, text, replyToMessageId, idempotencyKey);
    }
    /**
     * Send text as a post message (msg_type: 'post') with md tag.
     * Used for simple text — renders bold, italic, inline code, links.
     */
    async sendAsPost(chatId, text, replyToMessageId, idempotencyKey) {
        const postContent = buildPostContent(text);
        try {
            const res = replyToMessageId
                ? await this.restClient.im.message.reply({
                    path: { message_id: replyToMessageId },
                    data: { msg_type: 'post', content: postContent, uuid: idempotencyKey },
                })
                : await this.restClient.im.message.create({
                    params: { receive_id_type: 'chat_id' },
                    data: {
        ...
        try {
            const res = replyToMessageId
                ? await this.restClient.im.message.reply({
                    path: { message_id: replyToMessageId },
                    data: { msg_type: 'text', content: JSON.stringify({ text }), uuid: idempotencyKey },
                })
                : await this.restClient.im.message.create({
                    params: { receive_id_type: 'chat_id' },
                    data: {
     ...
     */
    async sendPermissionCard(chatId, text, inlineButtons, replyToMessageId, idempotencyKey) {
        if (!this.restClient) {
            return { ok: false, error: 'Feishu client not initialized' };
        }
        ...
        if (permId) {
            ...
            try {
                const res = replyToMessageId
                    ? await this.restClient.im.message.reply({
                        path: { message_id: replyToMessageId },
                        data: { msg_type: 'interactive', content: cardJson, uuid: idempotencyKey },
                    })
                    : await this.restClient.im.message.create({
                        params: { receive_id_type: 'chat_id' },
                        data: {
                ...
                console.warn('[feishu-adapter] Permission button card send failed:', JSON.stringify({ code: res?.code, msg: res?.msg }));
            }
            catch (err) {
        ...
        try {
            const res = replyToMessageId
                ? await this.restClient.im.message.reply({
                    path: { message_id: replyToMessageId },
                    data: { msg_type: 'text', content: JSON.stringify({ text: plainText }), uuid: idempotencyKey },
                })
                : await this.restClient.im.message.create({
                    params: { receive_id_type: 'chat_id' },
                    data: {
    ...
    async handleIncomingEvent(data) {
        try {
            ...
            console.error('[feishu-adapter] Unhandled error in event handler:', err instanceof Error ? err.stack || err.message : err);
        }
    }
    async processIncomingEvent(data) {
        ...
        if (sender.sender_type === 'bot')
            return;
        // Dedup by message_id
        if (this.seenMessageIds.has(msg.message_id))
            return;
        ...
        if (messageType === 'text') {
            ...
            const { fileKey, fileName } = this.extractResourceMetadata(msg.content);
            if (fileKey) {
        ...
        if (trimmedText.startsWith('/perm ')) {
            ...
            if (permParts.length >= 3) {
                ...
                const accepted = getBridgeContext().store.enqueueInboundMessage?.(inbound) ?? true;
                if (accepted)
     ...
     */
    extractFileKey(content) {
        return this.extractResourceMetadata(content).fileKey;
    }
    extractResourceMetadata(content) {
        try {
            const parsed = JSON.parse(content);
            return {
     ...
     */
    async resolveBotIdentity(appId, appSecret, domain) {
        try {
     ...
     */
    async downloadResource(messageId, fileKey, resourceType, originalName) {
        if (!this.restClient)
            return null;
        try {
            ...
            const fallbackMimeType = MIME_BY_TYPE[resourceType] || 'application/octet-stream';
            ...
            return {
                id,
                name: safeFileName(originalName || `${fileKey}.${ext}`),
                type: mimeFromFileName(originalName || '', fallbackMimeType),
                size: buffer.length,
                data: base64,
            };
        }
        catch (err) {
            console.error(`[feishu-adapter] Resource download failed (type=${resourceType}, key=${fileKey}):`, err instanceof Error ? err.stack || err.message : err);
            return null;
        }
    }
    // ── Utilities ───────────────────────────────────────────────
