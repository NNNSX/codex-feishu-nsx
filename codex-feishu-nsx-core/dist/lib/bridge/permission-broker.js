/**
 * Permission Broker — forwards provider permission requests to Feishu
 ...
 */
const recentPermissionForwards = new Map();
/**
 * Forward a permission request to Feishu as an interactive message.
 */
export async function forwardPermissionRequest(adapter, address, permissionRequestId, toolName, toolInput, sessionId, suggestions, replyToMessageId) {
    const { store } = getBridgeContext();
    // A persisted unresolved link means the request was already shown. Avoid
    // sending another card after a reconnect or recovery scan.
    const existingLink = store.getPermissionLink(permissionRequestId);
    if (existingLink && !existingLink.resolved) {
        return;
    }
    ...
    const message = {
        ...
        idempotencyKey: crypto.createHash('sha256')
            .update(`permission:${adapter.channelType}:${address.chatId}:${permissionRequestId}`)
            .digest('hex')
            .slice(0, 40),
    };
 ...
 */
export function handlePermissionCallback(callbackData, callbackChatId, callbackMessageId) {
    ...
    switch (action) {
        ...
        case 'allow_session': {
            ...
            if (link.suggestions) {
                try {
                    updatedPermissions = JSON.parse(link.suggestions);
                }
...
//# sourceMappingURL=permission-broker.js.map