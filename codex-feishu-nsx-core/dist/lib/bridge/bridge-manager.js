 ...
 */
function isNumericPermissionShortcut(channelType, rawText, chatId) {
...
function flushPreview(adapter, state, config) {
    if (state.degraded || !adapter.sendPreview)
        return;
 ...
 */
async function deliverResponse(adapter, address, responseText, sessionId, replyToMessageId, jobId) {
...
function nextDeliveryAttempt(attempt) {
    const delay = Math.min(5_000 * Math.pow(2, Math.max(0, attempt - 1)), DELIVERY_RETRY_MAX_DELAY_MS);
    return new Date(Date.now() + delay).toISOString();
}
async function deliverBridgeJob(adapter, job) {
    const { store } = getBridgeContext();
    const managerState = getState();
    if (!store.updateBridgeJob)
        return false;
    if (managerState.deliveryInFlight.has(job.id))
        return false;
    ...
    try {
        ...
        if (!current.textDelivered) {
            if (current.responseText) {
                const result = await deliverResponse(adapter, address, current.responseText, current.sessionId, current.replyToMessageId, current.id);
                if (result.ok) {
...
function markStartupJobsInterrupted() {
    const { store } = getBridgeContext();
    if (!store.listBridgeJobs || !store.updateBridgeJob)
        return;
...
async function reconcileBridgeJobs(adapter) {
    const { store } = getBridgeContext();
    if (!store.listBridgeJobs || !store.updateBridgeJob)
        return;
    for (const job of store.listBridgeJobs()) {
        if (!job.streamingCardId || job.streamingCardFinalized || ['received', 'running'].includes(job.state))
            continue;
        try {
            await adapter.finalizeRecoveredTask?.(job);
        }
        catch (error) {
    ...
    for (const job of store.listBridgeJobs(['codex_completed', 'delivering'])) {
        try {
            await deliverBridgeJob(adapter, job);
        }
        catch (error) {
    ...
    for (const job of store.listBridgeJobs(['interrupted', 'failed'])) {
        if (job.recoveryNoticeDelivered)
            continue;
        ...
        if (result.ok)
            store.updateBridgeJob(job.id, { recoveryNoticeDelivered: true });
    }
}
function getState() {
    const g = globalThis;
...
export async function start() {
    ...
    for (const channelType of getRegisteredTypes()) {
        ...
        if (store.getSetting(settingKey) !== 'true')
            continue;
        ...
        while (state.running && adapter.isRunning()) {
            try {
                ...
                if (msg.callbackData ||
                    ...
                    processWithSessionLock(binding.codepilotSessionId, () => handleMessage(adapter, msg)).catch(err => {
...
async function handleMessage(adapter, msg) {
    ...
    const ack = () => {
        try {
            store.removeInboundMessage?.(msg.messageId);
        }
        catch { /* best effort */ }
    ...
    try {
        ...
        const result = await engine.processMessage(binding, promptText, async (perm) => {
            await broker.forwardPermissionRequest(adapter, msg.address, perm.permissionRequestId, perm.toolName, perm.toolInput, binding.codepilotSessionId, perm.suggestions, msg.messageId);
        ...
        if (hasStreamingCards && adapter.onStreamEnd) {
            try {
                await adapter.onStreamEnd(msg.address.chatId, taskAbort.signal.aborted ? 'interrupted' : 'error', '');
            }
            catch { /* best effort */ }
        }
    }
 ...
 */
export function computeSdkSessionUpdate(sdkSessionId, hasError) {
